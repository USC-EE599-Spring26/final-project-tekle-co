...
//  OCKSample
//
//  Created by Kayal Bhatia on 4/8/26.
...
class CustomContactViewController: OCKListViewController, @unchecked Sendable {
	...
	func updateContacts() async throws {
        ...
        let filterdContacts = contacts.filter { convertedContact in
            let contactID = convertedContact.result.id
            let shouldIncludeContact = contactID.compare(personUUIDString) != .orderedSame
            Logger.contact.info("Contact filtered: \(contactID), included: \(shouldIncludeContact)")
            return shouldIncludeContact
        }
