...
private enum SurveyPresentationHost {
    ...
    /// Presents from the key window so ORK modals dismiss correctly (SwiftUI placeholder VCs are unreliable).
...
private final class ORKSurveyPresentationDelegate: NSObject, ObservableObject, ORKTaskViewControllerDelegate,
    @unchecked Sendable {
        ...
        DispatchQueue.main.async {
            ...
            Task {
                func finishAndNotify() async {
                    if kind == .onboard {
                        UserDefaults.standard.set(true, forKey: Constants.researchKitOnboardingCompletedKey)
                    }
                    await MainActor.run {
                        taskViewController.dismiss(animated: true) {
                ...
                guard let saveClosure else {
                    Logger.careKitTask.error("Survey save closure nil (CareKitTaskView onAppear may not have run)")
                    if kind == .onboard {
                        await finishAndNotify()
                    } else {
                        await MainActor.run {
...
struct CareKitTaskView: CareKitEssentialView {
    ...
    private var buttonTitle: String {
        event.isComplete ? String(localized: "COMPLETED") : String(localized: "START_SURVEY")
    }
}
